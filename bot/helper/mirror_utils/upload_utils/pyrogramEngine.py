...
class TgUploader:
    ...
    def __init__(self, name=None, listener=None):
        ...
        try:
            self.sent_msg = self.__app.copy_message(
                chat_id=listener.message.from_user.id,
                from_chat_id=listener.message.chat.id,
                message_id=listener.uid
            )
            print(self.sent_msg)
            app.delete_messages(listener.message.from_user.id, self.sent_msg.message_id)
            # app.edit_message_text(listener.message.from_user.id, self.sent_msg.message_id, "Leech command upload started")
    ...
    def upload(self):
        if self.is_chat_started:
            ...
            self.__listener.onUploadError(
                'Chat not started with the bot. Start the chat first before leaching here https://t.me/' + app.get_me().username + '?start=start')
